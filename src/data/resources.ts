import type { LearningResource } from "@/types/content";

const reviewed = "2026-08-31";
const scope = (en: string, bn: string) => ({ en, bn });

export const resources: LearningResource[] = [
  {
    id: "network-book",
    title: "An Introduction to Computer Networks",
    provider: "Peter L. Dordal · Loyola University Chicago",
    url: "https://intronetworks.cs.luc.edu/current2/html/",
    type: "book",
    isFree: true,
    recommendedScope: scope(
      "Read the overview, Ethernet, IP and transport introductions. Sketch a packet’s route before deeper routing chapters.",
      "Overview, Ethernet, IP আর transport introduction পড়ুন। গভীর routing chapter-এর আগে packet-এর পথ আঁকুন।",
    ),
    whyRecommended: scope(
      "A freely readable university-authored book that explains mechanisms rather than vendor command memorization.",
      "বিনা খরচে পড়া যায় এমন university-authored বই; vendor command মুখস্থ নয়, mechanism বোঝায়।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "wireshark-guide",
    title: "Wireshark User’s Guide",
    provider: "Wireshark Foundation",
    url: "https://www.wireshark.org/docs/wsug_html_chunked/",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Read capture prerequisites, filtering and following streams. Use only your own isolated lab or an explicitly authorized sample; captures may contain secrets.",
      "Capture prerequisite, filtering আর stream follow পড়ুন। শুধু নিজের বিচ্ছিন্ন lab বা স্পষ্ট অনুমোদিত sample ব্যবহার করুন; capture-এ secret থাকতে পারে।",
    ),
    whyRecommended: scope(
      "Official, free guidance for inspecting packet evidence and understanding the tool’s limits.",
      "Packet evidence দেখা আর tool-এর সীমা বোঝার official free guidance।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "kicad-start",
    title: "Getting Started in KiCad",
    provider: "KiCad",
    url: "https://docs.kicad.org/9.0/en/getting_started_in_kicad/getting_started_in_kicad.html",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Use the tutorial matching your installed version: schematic, electrical rules and board layout. Software/docs are free; components and fabrication are not included.",
      "Installed version-এর সঙ্গে মেলা tutorial নিন: schematic, electrical rule আর board layout। Software/docs free; component আর fabrication অন্তর্ভুক্ত নয়।",
    ),
    whyRecommended: scope(
      "Connects an editable circuit design with checks and fabrication outputs without requiring a board purchase.",
      "Board কেনা বাধ্যতামূলক না করে editable circuit design, check আর fabrication output যুক্ত করে।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "istqb-foundation",
    title: "Foundation Level testing syllabus",
    provider: "ISTQB",
    url: "https://istqb.org/certifications/certified-tester-foundation-level-ctfl-v4-0/",
    type: "article",
    isFree: true,
    recommendedScope: scope(
      "Read the freely downloadable syllabus sections on test design, risk and defect management. Training and the optional certification exam can cost money; neither is required here.",
      "Free downloadable syllabus-এর test design, risk আর defect management পড়ুন। Training আর optional certification exam-এ খরচ হতে পারে; এখানে কোনোটিই বাধ্যতামূলক নয়।",
    ),
    whyRecommended: scope(
      "Provides shared testing vocabulary; pair it with a real app and evidence rather than exam memorization.",
      "Testing-এর পরিচিত vocabulary দেয়; exam মুখস্থ নয়, real app আর evidence-এর সঙ্গে ব্যবহার করুন।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "playwright-testing",
    title: "Writing tests with Playwright",
    provider: "Microsoft · Playwright",
    url: "https://playwright.dev/docs/writing-tests",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Write one isolated user-journey test with meaningful locators and assertions; then test a failure state. Use the free local runner, not an optional hosted service.",
      "Meaningful locator আর assertion দিয়ে একটি isolated user-journey test লিখুন; তারপর failure state। Free local runner নিন, optional hosted service নয়।",
    ),
    whyRecommended: scope(
      "Teaches executable checks and observable outcomes without treating automation as the whole of QA.",
      "Automation-কে QA-এর সবকিছু না ধরে executable check আর দেখা যায় এমন outcome শেখায়।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "android-compose",
    title: "Android Basics with Compose",
    provider: "Android Developers",
    url: "https://developer.android.com/courses/android-basics-compose/course",
    type: "course",
    isFree: true,
    recommendedScope: scope(
      "Start with Kotlin and the first interactive app; then state and navigation. Choose this branch for Android, not alongside every other mobile stack.",
      "Kotlin আর প্রথম interactive app দিয়ে শুরু; তারপর state আর navigation। Android branch হিসেবে নিন, সব mobile stack-এর সঙ্গে একসঙ্গে নয়।",
    ),
    whyRecommended: scope(
      "An official learning sequence that connects language basics with real app behavior and tests.",
      "Language-এর basic থেকে app behavior আর test পর্যন্ত official শেখার sequence।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "apple-swift",
    title: "Develop in Swift",
    provider: "Apple Developer",
    url: "https://developer.apple.com/tutorials/develop-in-swift",
    type: "course",
    isFree: true,
    recommendedScope: scope(
      "Check tooling requirements first, then build one view and state transition. Tutorials are free; hardware and distribution may have costs.",
      "আগে tooling requirement দেখুন, তারপর একটি view আর state transition। Tutorial free; hardware আর distribution-এ খরচ হতে পারে।",
    ),
    whyRecommended: scope(
      "Official Swift and Apple-platform guidance instead of mixing outdated tutorials.",
      "পুরোনো tutorial মিশিয়ে ফেলার বদলে official Swift আর Apple-platform guidance।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "flutter",
    title: "Flutter documentation",
    provider: "Flutter",
    url: "https://docs.flutter.dev/install",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Pick your available target device, check setup and learn Dart/widgets/state for one small app. Paid hosted tools are unnecessary for this exercise.",
      "যে target device আছে সেটি বেছে setup দেখুন; ছোট app-এর জন্য Dart/widget/state শিখুন। এই exercise-এ paid hosted tool দরকার নেই।",
    ),
    whyRecommended: scope(
      "A maintained starting point for the Flutter branch, with platform-specific setup made explicit.",
      "Flutter branch-এর maintained starting point; platform-specific setup স্পষ্ট থাকে।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "react-native",
    title: "Get started with React Native",
    provider: "React Native",
    url: "https://reactnative.dev/docs/environment-setup",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Use if you already know JavaScript/React; read the framework and environment choices, then implement one screen. Check optional service costs separately.",
      "JavaScript/React জানা থাকলে framework আর environment-এর choice পড়ে একটি screen বানান। Optional service-এর খরচ আলাদা করে দেখুন।",
    ),
    whyRecommended: scope(
      "Explains the supported starting workflow without pretending cross-platform removes native constraints.",
      "Cross-platform মানেই native constraint নেই—এমন ধারণা না দিয়ে supported workflow বোঝায়।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "gov-user-research",
    title: "User research: Service Manual",
    provider: "GOV.UK",
    url: "https://www.gov.uk/service-manual/user-research",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Read consent and usability-testing guidance; plan one small observation session. This is a learning exercise, not a representative research study.",
      "Consent আর usability testing guidance পড়ে ছোট observation session plan করুন। এটি শেখার exercise, representative research study নয়।",
    ),
    whyRecommended: scope(
      "Connects research questions, ethical participation and evidence instead of invented personas.",
      "বানানো persona-এর বদলে research question, ethical participation আর evidence যুক্ত করে।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "gov-design-system",
    title: "GOV.UK Design System",
    provider: "Government Digital Service",
    url: "https://design-system.service.gov.uk/",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Study a form component and its error pattern. Learn the reasoning; do not copy government branding into your portfolio.",
      "একটি form component আর তার error pattern দেখুন। Reasoning শিখুন; portfolio-তে government branding copy করবেন না।",
    ),
    whyRecommended: scope(
      "Shows how research-informed patterns become documented, reusable interfaces.",
      "Research-informed pattern কীভাবে documented, reusable interface হয় দেখায়।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "cs50x",
    title: "CS50x: Introduction to Computer Science",
    provider: "Harvard University",
    url: "https://cs50.harvard.edu/x/",
    type: "university",
    isFree: true,
    recommendedScope: scope(
      "Weeks 0–5; finish the problem sets you can explain.",
      "Week 0–5; যে problem set solve করবে, সেটা যেন explain-ও করতে পারেন।",
    ),
    whyRecommended: scope(
      "A rigorous, friendly foundation in problem solving, C, memory and data structures.",
      "Problem solving, C, memory আর data structure-এর শক্ত foundation দেয়।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "git-book",
    title: "Pro Git",
    provider: "Git SCM",
    url: "https://git-scm.com/book/en/v2",
    type: "book",
    isFree: true,
    recommendedScope: scope(
      "Chapters 1–3 now; branching workflows later.",
      "এখন Chapter 1–3; advanced branching workflow পরে।",
    ),
    whyRecommended: scope(
      "The official, durable explanation of the tool Future You will wish you learned early.",
      "Git-এর official আর টেকসই ব্যাখ্যা—Future You এখন শিখে রাখার জন্য ধন্যবাদ দেবে।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "mdn",
    title: "MDN Learn Web Development",
    provider: "Mozilla",
    url: "https://developer.mozilla.org/en-US/docs/Learn_web_development",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Core HTML, CSS, JavaScript and accessibility modules.",
      "HTML, CSS, JavaScript আর accessibility-এর core moduleগুলো।",
    ),
    whyRecommended: scope(
      "Standards-first web learning without framework noise.",
      "Framework-এর শব্দ কমিয়ে web standard ঠিকভাবে শেখায়।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "web-a11y",
    title: "Web Accessibility Tutorials",
    provider: "W3C WAI",
    url: "https://www.w3.org/WAI/tutorials/",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Start with page structure, menus, images and forms.",
      "Page structure, menu, image আর form দিয়ে শুরু করুন।",
    ),
    whyRecommended: scope(
      "Authoritative guidance that turns accessibility into engineering practice.",
      "Accessibility-কে checkbox না বানিয়ে engineering practice হিসেবে শেখায়।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "node-learn",
    title: "Node.js Learn",
    provider: "OpenJS Foundation",
    url: "https://nodejs.org/en/learn",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "HTTP, asynchronous work, modules and testing basics.",
      "HTTP, async কাজ, module আর testing basics।",
    ),
    whyRecommended: scope(
      "An official path from JavaScript knowledge to server-side reasoning.",
      "JavaScript জানা থেকে server-side reasoning-এ যাওয়ার official পথ।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "postgres-tutorial",
    title: "PostgreSQL Tutorial",
    provider: "PostgreSQL Global Development Group",
    url: "https://www.postgresql.org/docs/current/tutorial.html",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Tables, queries, joins, aggregates and transactions.",
      "Table, query, join, aggregate আর transaction।",
    ),
    whyRecommended: scope(
      "Learn relational ideas from the database itself, not an ORM abstraction.",
      "ORM-এর আড়াল নয়, database থেকেই relational ধারণা শিখুন।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "sqlbolt",
    title: "SQLBolt",
    provider: "SQLBolt",
    url: "https://sqlbolt.com/",
    type: "practice",
    isFree: true,
    recommendedScope: scope(
      "Complete lessons 1–12 and the exercises.",
      "Lesson 1–12 আর exerciseগুলো করুন।",
    ),
    whyRecommended: scope(
      "Fast feedback for turning SQL syntax into usable skill.",
      "SQL syntax-কে কাজে লাগার skill বানাতে দ্রুত feedback দেয়।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "data-engineering-zoomcamp",
    title: "Data Engineering Zoomcamp",
    provider: "DataTalks.Club",
    url: "https://github.com/DataTalksClub/data-engineering-zoomcamp",
    type: "course",
    isFree: true,
    recommendedScope: scope(
      "Modules 1–3 first; orchestration and streaming can wait.",
      "আগে Module 1–3; orchestration আর streaming পরে হলেও চলবে।",
    ),
    whyRecommended: scope(
      "A realistic, project-oriented introduction to pipelines and warehouses.",
      "Pipeline আর warehouse-এর বাস্তব project-oriented পরিচয়।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "ml-course",
    title: "Machine Learning Crash Course",
    provider: "Google",
    url: "https://developers.google.com/machine-learning/crash-course",
    type: "course",
    isFree: true,
    recommendedScope: scope(
      "Regression, classification, data and overfitting modules.",
      "Regression, classification, data আর overfitting module।",
    ),
    whyRecommended: scope(
      "Concise theory linked to practical model behavior and evaluation.",
      "সংক্ষিপ্ত theory-কে model behavior আর evaluation-এর সঙ্গে জোড়া দেয়।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "kaggle-intro",
    title: "Intro to Machine Learning",
    provider: "Kaggle Learn",
    url: "https://www.kaggle.com/learn/intro-to-machine-learning",
    type: "practice",
    isFree: true,
    recommendedScope: scope(
      "Finish the short course and one notebook; skip competitions for now.",
      "Short course আর একটি notebook শেষ করুন; competition এখনই দরকার নেই।",
    ),
    whyRecommended: scope(
      "A low-friction way to train, validate and inspect a small model.",
      "ছোট model train, validate আর inspect করার কম-ঝামেলার পথ।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "portswigger",
    title: "Web Security Academy",
    provider: "PortSwigger",
    url: "https://portswigger.net/web-security",
    type: "lab",
    isFree: true,
    recommendedScope: scope(
      "Read the getting-started material and complete beginner SQLi/XSS labs only in the academy.",
      "Getting started পড়ে academy-র beginner SQLi/XSS lab করুন—শুধু authorized lab-এ।",
    ),
    whyRecommended: scope(
      "Safe, legal, deliberately vulnerable labs with excellent explanations.",
      "নিরাপদ, legal, ইচ্ছাকৃত vulnerable lab—ব্যাখ্যাও দারুণ।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "linux-journey",
    title: "Linux Journey",
    provider: "Linux Journey",
    url: "https://linuxjourney.com/",
    type: "course",
    isFree: true,
    recommendedScope: scope(
      "Grasshopper and Journeyman sections; type every command yourself.",
      "Grasshopper আর Journeyman section; command নিজে type করুন।",
    ),
    whyRecommended: scope(
      "A practical foundation before security or cloud tooling.",
      "Security বা cloud tool ধরার আগে দরকারি practical foundation।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "missing-semester",
    title: "The Missing Semester of Your CS Education",
    provider: "MIT",
    url: "https://missing.csail.mit.edu/",
    type: "university",
    isFree: true,
    recommendedScope: scope(
      "Shell, editors, data wrangling, Git and debugging lectures.",
      "Shell, editor, data wrangling, Git আর debugging lecture।",
    ),
    whyRecommended: scope(
      "The everyday engineering tools many curricula leave implicit.",
      "যে everyday engineering toolগুলো curriculum প্রায়ই ধরে নেয় আপনি জানুন।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "docker-start",
    title: "Docker Get Started",
    provider: "Docker",
    url: "https://docs.docker.com/get-started/",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Containerize one small app and understand images, containers, ports and volumes.",
      "একটি ছোট app containerize করে image, container, port আর volume বুঝুন।",
    ),
    whyRecommended: scope(
      "Enough container literacy for a first deployment—no Kubernetes detour.",
      "প্রথম deployment-এর জন্য যথেষ্ট container literacy—Kubernetes detour ছাড়াই।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "nand2tetris",
    title: "Nand2Tetris",
    provider: "Hebrew University of Jerusalem",
    url: "https://www.nand2tetris.org/",
    type: "university",
    isFree: true,
    recommendedScope: scope(
      "Projects 1–5 to connect gates, CPU and machine code.",
      "Project 1–5 করে gate, CPU আর machine code-এর যোগটা বুঝুন।",
    ),
    whyRecommended: scope(
      "Builds a concrete mental model of what lies beneath software.",
      "Software-এর নিচে আসলে কী আছে—তার concrete mental model বানায়।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "arduino-docs",
    title: "Arduino Documentation",
    provider: "Arduino",
    url: "https://docs.arduino.cc/learn/",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Digital/analog I/O, sensors and one small timing-based project.",
      "Digital/analog I/O, sensor আর একটি ছোট timing-based project।",
    ),
    whyRecommended: scope(
      "Immediate hardware feedback without hiding basic embedded constraints.",
      "Basic embedded constraint না লুকিয়ে দ্রুত hardware feedback দেয়।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "algorithms-princeton",
    title: "Algorithms, Part I",
    provider: "Princeton University",
    url: "https://www.coursera.org/learn/algorithms-part1",
    type: "university",
    isFree: true,
    recommendedScope: scope(
      "Audit core data structures, sorting and graph foundations.",
      "Core data structure, sorting আর graph foundation audit করুন।",
    ),
    whyRecommended: scope(
      "A rigorous bridge from solving problems to reasoning about algorithms.",
      "Problem solve করা থেকে algorithm নিয়ে rigorously ভাবার সেতু।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "teach-yourself-cs",
    title: "Teach Yourself Computer Science",
    provider: "TeachYourselfCS",
    url: "https://teachyourselfcs.com/",
    type: "article",
    isFree: true,
    recommendedScope: scope(
      "Use the subject map; choose one deep subject, not all at once.",
      "Subject map ব্যবহার করুন; একসঙ্গে সব নয়, একটি deep subject বেছে নিন।",
    ),
    whyRecommended: scope(
      "A curated map for students who want depth beyond tool tutorials.",
      "Tool tutorial-এর বাইরে depth চাইলে শক্ত curated map।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "accidental-cto",
    title: "The Accidental CTO",
    provider: "Subhash Chy",
    url: "https://github.com/subhashchy/The-Accidental-CTO/blob/main/The%20Accidental%20CTO.md",
    type: "book",
    isFree: true,
    recommendedScope: scope(
      "Read selectively after basic backend and systems foundations. Treat the incident-driven chapters as prompts to compare scaling and reliability trade-offs—not as a universal architecture recipe.",
      "Backend আর system-এর basic foundation হওয়ার পরে বেছে বেছে পড়ুন। Incident-ভিত্তিক chapterগুলো দিয়ে scaling ও reliability-র trade-off ভাবুন; সব project-এ বসিয়ে দেওয়ার architecture recipe হিসেবে নেবেন না।",
    ),
    whyRecommended: scope(
      "A free, story-driven tour of how technical and leadership decisions evolve as a product grows, useful for connecting isolated tools to operational consequences.",
      "Product বড় হওয়ার সঙ্গে technical আর leadership decision কীভাবে বদলায়, সেটি গল্পের ভেতর দিয়ে দেখায়। আলাদা আলাদা tool-এর সঙ্গে production consequence-এর যোগসূত্র ধরতে কাজে লাগে।",
    ),
    lastReviewed: "2026-09-08",
    language: "en",
  },
  {
    id: "refactoring-guru",
    title: "Clean Code and Refactoring",
    provider: "Refactoring.Guru",
    url: "https://refactoring.guru/refactoring/what-is-refactoring",
    type: "article",
    isFree: true,
    recommendedScope: scope(
      "Read the clean-code characteristics, then improve names, duplication or one oversized function in working code. Protect behavior with tests before a larger refactor.",
      "Clean code-এর বৈশিষ্ট্যগুলো পড়ে working code-এ naming, duplication বা বড় একটি function improve করুন। বড় refactor-এর আগে test দিয়ে behavior ধরে রাখুন।",
    ),
    whyRecommended: scope(
      "Connects readability, simplicity, duplication and tests to the real maintenance cost of code—including code drafted with AI.",
      "Readability, simplicity, duplication আর test-কে code maintain করার বাস্তব খরচের সঙ্গে যুক্ত করে—AI দিয়ে draft করা code-ও এর বাইরে নয়।",
    ),
    lastReviewed: "2026-09-09",
    language: "en",
  },
  {
    id: "code-smells",
    title: "Code Smells",
    provider: "Refactoring.Guru",
    url: "https://refactoring.guru/refactoring/smells",
    type: "article",
    isFree: true,
    recommendedScope: scope(
      "Begin with Bloaters, Change Preventers and Couplers. Find one example in your own project, describe its change cost and refactor only with behavior protected.",
      "Bloaters, Change Preventers আর Couplers দিয়ে শুরু করুন। নিজের project-এ একটি example খুঁজে change cost লিখুন; behavior protect করে তবেই refactor করুন।",
    ),
    whyRecommended: scope(
      "A visual vocabulary for spotting maintainability pressure that compiling and AI-generated confidence do not reveal.",
      "Compile হওয়া বা AI-এর আত্মবিশ্বাসী output যে maintainability pressure দেখায় না, সেটি ধরার visual vocabulary দেয়।",
    ),
    lastReviewed: "2026-09-09",
    language: "en",
  },
  {
    id: "design-patterns",
    title: "Design Patterns",
    provider: "Refactoring.Guru",
    url: "https://refactoring.guru/design-patterns",
    type: "article",
    isFree: true,
    recommendedScope: scope(
      "Read what a pattern is, its benefits and criticism. Study Strategy or Adapter only after you can point to that problem in working code; compare it with the simpler option.",
      "Pattern কী, লাভ কী আর criticism কোথায়—আগে এগুলো পড়ুন। Working code-এ সমস্যা দেখাতে পারলে তবেই Strategy বা Adapter দেখুন; simpler option-এর সঙ্গে compare করুন।",
    ),
    whyRecommended: scope(
      "Explains patterns as adaptable design blueprints and shared team language, while also making room for their limits and misuse.",
      "Pattern-কে adaptable design blueprint আর team-এর shared language হিসেবে বোঝায়; একই সঙ্গে সীমা আর misuse নিয়েও ভাবায়।",
    ),
    lastReviewed: "2026-09-09",
    language: "en",
  },
  {
    id: "testing-js",
    title: "JavaScript Testing Best Practices",
    provider: "Community-maintained open guide",
    url: "https://github.com/goldbergyoni/javascript-testing-best-practices",
    type: "book",
    isFree: true,
    recommendedScope: scope(
      "Read testing fundamentals and apply three tests to a small app.",
      "Testing foundation পড়ে ছোট app-এ তিনটি meaningful test দিন।",
    ),
    whyRecommended: scope(
      "Practical guidance on behavior-focused tests and maintainability.",
      "Behavior-focused test আর maintainability নিয়ে practical guidance।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
  {
    id: "kaggle-pandas",
    title: "Pandas",
    provider: "Kaggle Learn",
    url: "https://www.kaggle.com/learn/pandas",
    type: "practice",
    isFree: true,
    recommendedScope: scope(
      "Complete the short exercises, then repeat one analysis on a small CSV you understand.",
      "Short exercise শেষ করে নিজের বোঝা ছোট CSV-তে একটি analysis আবার করুন।",
    ),
    whyRecommended: scope(
      "Quick practice for selecting, cleaning, grouping and checking tabular data.",
      "Tabular data select, clean, group আর check করার দ্রুত practice।",
    ),
    lastReviewed: "2026-09-01",
    language: "en",
  },
  {
    id: "openintro-statistics",
    title: "OpenIntro Statistics",
    provider: "OpenIntro",
    url: "https://www.openintro.org/book/os/",
    type: "book",
    isFree: true,
    recommendedScope: scope(
      "Use the free text for distributions, study design, inference and the difference between association and causation.",
      "Distribution, study design, inference আর association-causation পার্থক্যের জন্য free text ব্যবহার করুন।",
    ),
    whyRecommended: scope(
      "A rigorous, openly available statistics foundation for honest analysis.",
      "সৎ analysis-এর জন্য rigorous, openly available statistics foundation।",
    ),
    lastReviewed: "2026-09-01",
    language: "en",
  },
  {
    id: "power-bi-learning",
    title: "Power BI learning path",
    provider: "Microsoft Learn",
    url: "https://learn.microsoft.com/en-us/training/powerplatform/power-bi/",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Use the beginner modules for data preparation, modeling and accessible reports; one BI tool is enough to start.",
      "Data preparation, modeling আর accessible report-এর beginner module দেখুন; শুরুতে একটি BI tool-ই যথেষ্ট।",
    ),
    whyRecommended: scope(
      "Official, task-oriented guidance that connects a report to its data model.",
      "Report-কে data model-এর সঙ্গে যুক্ত করা official task-oriented guidance।",
    ),
    lastReviewed: "2026-09-01",
    language: "en",
  },
  {
    id: "hf-llm-course",
    title: "LLM Course",
    provider: "Hugging Face",
    url: "https://huggingface.co/learn/llm-course/chapter1/1",
    type: "course",
    isFree: true,
    recommendedScope: scope(
      "Study model and transformer foundations before using the later application chapters selectively.",
      "পরে application chapter বেছে নেওয়ার আগে model আর transformer foundation পড়ুন।",
    ),
    whyRecommended: scope(
      "A free technical foundation that makes model behavior less mysterious.",
      "Model behavior-কে কম mysterious করে এমন free technical foundation।",
    ),
    lastReviewed: "2026-09-01",
    language: "en",
  },
  {
    id: "full-stack-deep-learning",
    title: "Full Stack Deep Learning",
    provider: "Full Stack Deep Learning",
    url: "https://fullstackdeeplearning.com/course/2022/",
    type: "course",
    isFree: true,
    recommendedScope: scope(
      "Use selected lectures on development infrastructure, testing, deployment and continual learning; verify current tool details separately.",
      "Development infrastructure, testing, deployment আর continual learning-এর lecture বেছে দেখুন; current tool detail আলাদা যাচাই করুন।",
    ),
    whyRecommended: scope(
      "Connects models to the engineering work required around production systems.",
      "Model-কে production system-এর চারপাশের engineering work-এর সঙ্গে যুক্ত করে।",
    ),
    lastReviewed: "2026-09-01",
    language: "en",
  },
  {
    id: "made-with-ml",
    title: "Made With ML",
    provider: "Made With ML",
    url: "https://madewithml.com/",
    type: "course",
    isFree: true,
    recommendedScope: scope(
      "Focus on data, evaluation, testing and production lessons; reproduce a small example rather than copying the full stack.",
      "Data, evaluation, testing আর production lesson-এ focus করুন; পুরো stack copy না করে ছোট example reproduce করুন।",
    ),
    whyRecommended: scope(
      "Treats evaluation and reliable delivery as part of ML/AI engineering.",
      "Evaluation আর reliable delivery-কে ML/AI engineering-এর অংশ হিসেবে ধরে।",
    ),
    lastReviewed: "2026-09-01",
    language: "en",
  },
  {
    id: "amirul-ai-roadmap",
    title: "AI Engineering Roadmap",
    provider: "Amirul Islam Al Mamun",
    url: "https://amirulislamalmamun.com/practice/ai-engineering/roadmap/",
    type: "article",
    isFree: true,
    recommendedScope: scope(
      "Use the staged path to understand prompts, retrieval, evaluation and production concerns; treat dated provider details as changeable.",
      "Prompt, retrieval, evaluation আর production concern বুঝতে staged path ব্যবহার করুন; dated provider detail বদলাতে পারে।",
    ),
    whyRecommended: scope(
      "A practical engineer-to-engineer view that ends stages with something built instead of a certificate.",
      "Certificate নয়, stage শেষে কিছু build করার practical engineer-to-engineer দৃষ্টিভঙ্গি।",
    ),
    lastReviewed: "2026-09-01",
    language: "en",
  },
  {
    id: "openai-building-agents",
    title: "A practical guide to building agents",
    provider: "OpenAI",
    url: "https://openai.com/business/guides-and-resources/a-practical-guide-to-building-ai-agents/",
    type: "article",
    isFree: true,
    recommendedScope: scope(
      "Read what qualifies as an agent, when a deterministic workflow is enough, the single-agent loop, tool design, guardrails and human intervention. Treat SDK examples as one implementation, not the concept itself.",
      "কোন system agent, কখন deterministic workflow যথেষ্ট, single-agent loop, tool design, guardrail আর human intervention পড়ুন। SDK example-কে concept নয়, একটি implementation হিসেবে ধরুন।",
    ),
    whyRecommended: scope(
      "A primary guide that puts workflow fit, evals, bounded tools and guardrails before multi-agent complexity.",
      "Multi-agent complexity-এর আগে workflow fit, eval, bounded tool আর guardrail রাখে এমন primary guide।",
    ),
    lastReviewed: "2026-09-07",
    language: "en",
  },
  {
    id: "google-rag-overview",
    title: "What is Retrieval-Augmented Generation (RAG)?",
    provider: "Google Cloud",
    url: "https://cloud.google.com/use-cases/retrieval-augmented-generation",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Learn the retrieval → augmentation → generation flow and why data ingestion, retrieval quality, grounding and serving must be reasoned about separately.",
      "Retrieval → augmentation → generation flow আর data ingestion, retrieval quality, grounding ও serving কেন আলাদা করে ভাবতে হয় শিখুন।",
    ),
    whyRecommended: scope(
      "A maintained conceptual overview that separates RAG architecture from a fashionable vector-database demo.",
      "RAG architecture-কে fashionable vector-database demo থেকে আলাদা করা maintained conceptual overview।",
    ),
    lastReviewed: "2026-09-07",
    language: "en",
  },
  {
    id: "n8n-learning-path",
    title: "n8n learning paths",
    provider: "n8n",
    url: "https://docs.n8n.io/learning-paths/",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Learn workflow structure, data flow, branches, loops, error handling and executions before adding an AI node. Rebuild one small, harmless workflow and inspect its failure history.",
      "AI node-এর আগে workflow structure, data flow, branch, loop, error handling আর execution শিখুন। একটি harmless small workflow rebuild করে failure history দেখুন।",
    ),
    whyRecommended: scope(
      "Official tool guidance for implementing visible workflows without confusing a canvas with engineering judgment.",
      "Canvas-কে engineering judgment না ভেবে visible workflow implement করার official tool guidance।",
    ),
    lastReviewed: "2026-09-07",
    language: "en",
  },
  {
    id: "openclaw-agent-runtime",
    title: "OpenClaw agent runtime and security model",
    provider: "OpenClaw",
    url: "https://docs.openclaw.ai/agent",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "After building one bounded agent, inspect its loop, workspace, tool wiring and sessions; then read the linked security, sandbox and tool-policy guidance before granting host or account access.",
      "একটি bounded agent বানানোর পরে loop, workspace, tool wiring আর session দেখুন; host বা account access দেওয়ার আগে linked security, sandbox আর tool-policy guidance পড়ুন।",
    ),
    whyRecommended: scope(
      "A concrete runtime example that makes persistent agent authority—and the resulting trust boundary—visible.",
      "Persistent agent authority এবং তার trust boundary-কে visible করা concrete runtime example।",
    ),
    lastReviewed: "2026-09-07",
    language: "en",
  },
  {
    id: "cisa-cyber-defense-analyst",
    title: "Cyber Defense Analyst work role",
    provider: "CISA",
    url: "https://www.cisa.gov/careers/work-rolescyber-defense-analyst",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Read the role purpose, core tasks and knowledge list. Use it to distinguish evidence-based monitoring and escalation from the movie version of a SOC.",
      "Role purpose, core task আর knowledge list পড়ুন। Evidence-based monitoring ও escalation-কে সিনেমার SOC থেকে আলাদা করে বুঝতে এটি ব্যবহার করুন।",
    ),
    whyRecommended: scope(
      "An official task-level description of defensive analysis work, including alert correlation, documentation and escalation.",
      "Alert correlation, documentation আর escalation-সহ defensive analysis কাজের official task-level description।",
    ),
    lastReviewed: "2026-09-14",
    language: "en",
  },
  {
    id: "nist-incident-response",
    title: "NIST SP 800-61 Rev. 3 — Incident Response",
    provider: "NIST",
    url: "https://csrc.nist.gov/pubs/sp/800/61/r3/final",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Start with the executive summary and incident-response life-cycle considerations. Map preparation, detection, response and recovery to one fictional incident; do not treat the publication as a tool tutorial.",
      "Executive summary আর incident-response life-cycle consideration দিয়ে শুরু করুন। একটি fictional incident-এ preparation, detection, response আর recovery map করুন; publication-টিকে tool tutorial ভাববেন না।",
    ),
    whyRecommended: scope(
      "Current, vendor-neutral incident-response guidance that keeps detection and recovery connected to organizational risk.",
      "Current vendor-neutral incident-response guidance; detection ও recovery-কে organizational risk-এর সঙ্গে যুক্ত রাখে।",
    ),
    lastReviewed: "2026-09-14",
    language: "en",
  },
  {
    id: "mitre-attack-start",
    title: "Get Started with MITRE ATT&CK",
    provider: "MITRE",
    url: "https://attack.mitre.org/resources/",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Learn tactics, techniques, procedures and the detection/analytics use case. Map only a few relevant behaviors to available log evidence; never chase 100% matrix coverage.",
      "Tactic, technique, procedure আর detection/analytics use case বুঝুন। Available log evidence-এর সঙ্গে অল্প কয়েকটি relevant behavior map করুন; 100% matrix coverage-এর পেছনে ছুটবেন না।",
    ),
    whyRecommended: scope(
      "A maintained common language for reasoning about observed adversary behavior, detections and defensive gaps.",
      "Observed adversary behavior, detection আর defensive gap নিয়ে ভাবার maintained common language।",
    ),
    lastReviewed: "2026-09-14",
    language: "en",
  },
  {
    id: "owasp-wstg-v42",
    title: "OWASP Web Security Testing Guide v4.2",
    provider: "OWASP Foundation",
    url: "https://wstg.owasp.org/v4.2/",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Read the introduction and testing framework, then choose only the versioned scenarios relevant to one explicitly authorized training application. Record scope, evidence and limits.",
      "Introduction আর testing framework পড়ে explicitly authorized training application-এর জন্য relevant versioned scenario বাছুন। Scope, evidence আর limit লিখে রাখুন।",
    ),
    whyRecommended: scope(
      "A vendor-neutral testing methodology that frames penetration testing as scoped investigation and reporting—not indiscriminate scanning.",
      "Vendor-neutral testing methodology; penetration testing-কে random scanning নয়, scoped investigation আর reporting হিসেবে শেখায়।",
    ),
    lastReviewed: "2026-09-14",
    language: "en",
  },
  {
    id: "owasp-devsecops",
    title: "OWASP DevSecOps Guideline",
    provider: "OWASP Foundation",
    url: "https://devguide.owasp.org/en/09-operations/01-devsecops/",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Read the pipeline overview, threat modelling, secrets and scanning sections. Add one reviewed check to a practice pipeline and document false-positive handling and a safe exception path.",
      "Pipeline overview, threat modelling, secret আর scanning section পড়ুন। Practice pipeline-এ একটি reviewed check যোগ করে false positive handling ও safe exception path লিখুন।",
    ),
    whyRecommended: scope(
      "Connects security activities to delivery stages while making clear that tools support—not replace—engineering review.",
      "Security activity-কে delivery stage-এর সঙ্গে যুক্ত করে, আর স্পষ্ট রাখে—tool engineering review-কে সাহায্য করে, replace করে না।",
    ),
    lastReviewed: "2026-09-14",
    language: "en",
  },
  {
    id: "owasp-top-ten",
    title: "OWASP Top 10",
    provider: "OWASP Foundation",
    url: "https://owasp.org/www-project-top-ten/",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Use the categories to recognize common application risks, then verify behavior in an authorized lab rather than scanning public targets.",
      "Common application risk চিনতে category ব্যবহার করুন; public target scan নয়, authorized lab-এ behavior verify করুন।",
    ),
    whyRecommended: scope(
      "A shared starting vocabulary for application-security risk, not a complete test plan.",
      "Application-security risk-এর shared starting vocabulary; complete test plan নয়।",
    ),
    lastReviewed: "2026-09-01",
    language: "en",
  },
  {
    id: "owasp-cheat-sheets",
    title: "OWASP Cheat Sheet Series",
    provider: "OWASP Foundation",
    url: "https://cheatsheetseries.owasp.org/",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Select the sheet matching one feature, such as input validation or authentication; test the recommendation in your own small app.",
      "Input validation বা authentication-এর মতো একটি feature-এর matching sheet নিন; নিজের small app-এ recommendation test করুন।",
    ),
    whyRecommended: scope(
      "Actionable prevention guidance organized by the problem developers are implementing.",
      "Developer যে problem implement করছে সেই অনুযায়ী actionable prevention guidance।",
    ),
    lastReviewed: "2026-09-01",
    language: "en",
  },
  {
    id: "owasp-threat-modeling",
    title: "Threat Modeling Cheat Sheet",
    provider: "OWASP Foundation",
    url: "https://cheatsheetseries.owasp.org/cheatsheets/Threat_Modeling_Cheat_Sheet.html",
    type: "official-docs",
    isFree: true,
    recommendedScope: scope(
      "Map one small feature's data flow, trust boundaries, threats and mitigations with its owners.",
      "Owner-দের সঙ্গে একটি small feature-এর data flow, trust boundary, threat আর mitigation map করুন।",
    ),
    whyRecommended: scope(
      "Moves security earlier than scanner output and keeps it tied to system context.",
      "Security-কে scanner output-এর আগেই আনে আর system context-এর সঙ্গে যুক্ত রাখে।",
    ),
    lastReviewed: "2026-09-01",
    language: "en",
  },
  {
    id: "shafaet-algorithms-bn",
    title: "শাফায়েতের অ্যালগরিদম লেখা",
    provider: "Shafaet Ashraf",
    url: "https://www.shafaetsplanet.com/?page_id=2804",
    type: "article",
    isFree: true,
    recommendedScope: scope(
      "Start with complexity, basic data structures and graph foundations; attempt a problem before reading an advanced explanation.",
      "Complexity, basic data structure আর graph foundation দিয়ে শুরু করুন; advanced explanation পড়ার আগে problem চেষ্টা করুন।",
    ),
    whyRecommended: scope(
      "A broad Bangla index of programming, data-structure and algorithm explanations grounded in problem solving.",
      "Problem solving-ভিত্তিক programming, data structure আর algorithm-এর বিস্তৃত বাংলা index।",
    ),
    lastReviewed: "2026-09-01",
    language: "bn",
  },
  {
    id: "amirul-programming-bn",
    title: "বাংলা programming ও competitive-programming লেখা",
    provider: "Amirul Islam Al Mamun",
    url: "https://amirulislamalmamun.com/categories/%E0%A6%AC%E0%A6%BE%E0%A6%82%E0%A6%B2%E0%A6%BE/",
    type: "article",
    isFree: true,
    recommendedScope: scope(
      "Use the Bangla category selectively for competitive-programming motivation, practice reflection and related engineering notes.",
      "Competitive-programming motivation, practice reflection আর related engineering note-এর জন্য বাংলা category বেছে ব্যবহার করুন।",
    ),
    whyRecommended: scope(
      "Adds a Bangladesh-based practitioner perspective alongside structured algorithm references.",
      "Structured algorithm reference-এর পাশে Bangladesh-based practitioner perspective যোগ করে।",
    ),
    lastReviewed: "2026-09-01",
    language: "bn",
  },
  {
    id: "cp-algorithms",
    title: "CP-Algorithms",
    provider: "Community-maintained open reference",
    url: "https://cp-algorithms.com/",
    type: "practice",
    isFree: true,
    recommendedScope: scope(
      "Use as a reference after attempting problems, not as a list to memorize.",
      "Problem চেষ্টা করার পরে reference হিসেবে ব্যবহার করুন; মুখস্থের checklist না।",
    ),
    whyRecommended: scope(
      "Clear algorithm explanations for the competence and competitive tracks.",
      "Competence আর competitive track-এর জন্য পরিষ্কার algorithm reference।",
    ),
    lastReviewed: reviewed,
    language: "en",
  },
];

export const resourceById = new Map(
  resources.map((resource) => [resource.id, resource]),
);
