...
8. “Try before you choose”: three immediately visible representative previews plus Data Analytics & BI, AI Engineering and Application Security in a native disclosure, linking to the full published experiment index (currently nineteen); derive the displayed count from data.
...
Render reusable two-item contextual subsets in the new-student, feel-behind, first-role, software-foundations and AI guides. Resolve IDs against the same typed source, fail the build for unknown IDs, retain safe external-link analytics, and link every subset to the full sequence. Contextual media must stay supplemental to the page action rather than becoming an autoplay feed or a substitute for technical evidence.
...
surround it with editorial commentary about optimization, reproducibility or
token efficiency; keep the external prompting reference label concise. Do not
...
Dark is the first-visit default. Apply it in the server-rendered HTML and inline theme-color bootstrap so the initial paint does not flash light; honor and persist an explicit stored light choice.
...
Visible attribution is implemented as static shared-footer content and localized About `#credits`, backed by typed bilingual `src/data/credits.ts` and the creator identity/portfolio URL in `src/config/site.ts`. Render Amjad Hossain's name as a new-tab link to the intended GitHub Pages portfolio at `https://amjadhossainrahat.github.io/` with safe `rel` attributes; retain this canonical destination even while the portfolio is not yet published. Keep all three inspiration URLs (roadmap.sh, Architect Prep and Amirul Islam) and the distinct roles: Amjad contributed **Idea, Prompting, AI Agent Orchestrating, Review and Quality Control**; ChatGPT handled idea polishing plus requirement/design analysis and finalization; Codex using **GPT-5.6 Sol (High)** and **GPT-6 Astra (High)** handled implementation and iteration. Protect these credits with content-integrity coverage. Verify footer-to-About base-path links, creator-link semantics, keyboard/no-JavaScript use, English/Bangla, both themes and narrow-screen wrapping. Build verification must confirm footer credits on all generated HTML pages and the target anchors. Do not add client hydration or analytics just for credits.
...
Maintain nineteen complete careers across eight families. Mobile App Development and UX/UI Engineering must participate in career comparison, families, five-stage roadmaps, experiments, AI pages and bilingual route generation. Data Analytics & BI, AI Engineering, Security Operations, Offensive Security and Application Security require the same complete graph and role-specific professional stages. Use role-specific stage overrides where generic software tasks or JavaScript testing resources would be misleading. Preserve existing routes and comparison defaults when adding entries.
...
Test all seven routes in both languages, the two new careers across career/roadmap/experiment/AI routes, homepage → goal → career-roadmap and lost-flow journeys, equivalent language switching, keyboard/no-JavaScript disclosure, 390/768/1280/1600px layouts, both themes, console errors and relevant axe checks. The responsive support and test contract begins at 390px. Cover goal/source analytics with the existing bounded `next_step_clicked` and `learning_resource_clicked` abstraction. Extend external-link checks to the guidance-source registry. Run both root and project-Pages production verification; inspect actual viewport screenshots and retain truthful validation records.

## 45. Definition of Done

### Final-year project and infrastructure-career requirements

Publish nineteen complete careers. `infrastructure-careers.ts`, `infrastructure-experiments.ts` and `infrastructure-roadmaps.ts` carry Network Engineering, Hardware Engineering and Software QA & Testing with role-specific evidence, safe experiments and professional practices. `specialist-careers.ts`, `specialist-experiments.ts` and `specialist-roadmaps.ts` carry Data Analytics & BI, AI Engineering, Security Operations, Offensive Security and Application Security with reproducible metrics, evaluation/release evidence, explicitly authorized exercises and responsible remediation. DevSecOps remains a visible delivery-security specialization, while QA visibly separates exploratory, automation/SDET, performance, accessibility and security-testing work. Keep stable IDs, exact family membership, all relationships and natural English/Bangla content. Do not inherit a generic browser-code task or JavaScript-only testing resource where it would misrepresent physical, operational, analytical, AI or security engineering.
